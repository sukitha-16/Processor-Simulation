...
n_ldst_nonzero_offset = 0 #Total non-zero offset loadstore instructions

n_loads=0		#Total load instructions retired while collecting_stats ==True
n_stores=0		#Total store instructions retired while collecting_stats ==True 
...
% Arithmetic  : {arith:.3f} %
% Ld/St       : {ldst:.3f} %
% Ld/St_with_nonzero_offset  : {ldst_with_nonzero_offset:.3f} %
...
""".format(cpi=n_cycles / n_instructions,
           ...
           ldst_with_nonzero_offset=100*n_ldst_nonzero_offset/n_instructions,
           brjmp=100 * n_brjmp_instructions / n_instructions,
           misc=100 * n_misc_instructions / n_instructions))
           
           
increase = (n_ldst_nonzero_offset/n_instructions)*(100*n_ldst_instructions/n_instructions)

print("Expected increase in instruction count : ",increase)

if increase<=5:
	print("I would recommend adopting new design since the percentage increase in total number of instructions executed is within 5%.")
else:
	print("I would not recommend adopting new design since the percentage increase in total number of instructions executed is More than 5%.")
